...
const subjects = [
  { name: "Structured Programming Laboratory", credits: 1.5 },
  { name: "Developing Language Skills II", credits: 3 },
  { name: "Emergence and History of Bangladesh", credits: 3 },
  { name: "Calculus I", credits: 3 },
  { name: "Structured Programming", credits: 3 },
];
...
export default function Semester2() {
  ...
  const calculateSubjectTotal = (item) => {
    if (["Structured Programming Laboratory"].includes(item.name)) {
  ...
  const handleDownloadPDF = () => {
    const doc = new jsPDF();
    doc.setFontSize(18);
    doc.text("CGPA Report – 2514 CSE Batch (2nd Semester)", 14, 20);
  ...
  return (
    <div>
      <div className="max-w-8xl mx-auto mt-8 p-4 sm:p-6 bg-white shadow rounded">
        <SectionTitle
          heading="Batch 2514 – Simplified CGPA Calculator (2nd Semester)"
          subHeading={`This tool is specially designed for the 2nd semester based on fixed course subjects. Add your Name and ID to personalize the PDF report.`}
        />
        ...
        {data.map((item, idx) => (
          <div key={idx} className="mb-6 border-b pb-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3">
              {["Structured Programming Laboratory"].includes(item.name) ? (
                <input
                  type="number"
